...
const fr: Dict = {
  ...
  findings: {
    ...
    intro: (canton) => `Quatre choses à savoir sur les Chiliens de ${canton} — puis les données elles-mêmes, à explorer librement. Quand un chiffre n'a jamais été publié, cette page le dit au lieu d'afficher un vide.`,
    twoCountsH: "Deux comptages, une communauté",
    twoCountsD: (p, b) => `${p} personnes ont un passeport chilien ; ${b} sont nées au Chili. Ne compter que l'un des deux fait manquer l'essentiel de l'autre.`,
    twoCountsCta: "Voir la répartition",
    becameSwissH: "Sont devenus suisses",
    becameSwissD: (b, swiss, latam) => `Sur les ${b} personnes nées au Chili, ${swiss} ont aujourd'hui un passeport suisse et ${latam} un passeport latino-américain.`,
    ...
    over65D: (p, recent) => `Sur ${p} détenteurs de passeport${recent ? `, dont ${recent} arrivés ces cinq dernières années` : ""}.`,
    over65Cta: "Voir le portrait",
  },
  hero: {
    eyebrow: "Deux façons de compter",
    h: "La communauté est plus grande que le compte des passeports",
    lead: (canton, p, b) => `${canton} compte ${p} détenteurs d'un passeport chilien — et ${b} résidents nés au Chili. Beaucoup des personnes nées au Chili ont depuis pris la nationalité suisse ou d'un pays de l'UE ; ne compter que les passeports fait manquer une grande partie de la communauté.`,
    kickerPassport: "Ont un passeport chilien",
    kickerBorn: "Sont nés au Chili",
    footSem: (date) => `SEM · ${date} · résidents permanents`,
    footBfs: "BFS STATPOP · 31 déc 2024 · résidents permanents",
    splitHead: (b) => `Les ${b} résidents nés au Chili, selon le passeport qu'ils détiennent réellement`,
    splitAria: "Composition par passeport des résidents nés au Chili",
    insight: (nonLatAm, born, pct) => `${nonLatAm} des ${born} résidents nés au Chili — ${pct}% — détiennent un passeport autre que latino-américain. La naturalisation et les familles binationales font fortement diverger lieu de naissance et nationalité à cette échelle.`,
  ...
  portrait: {
    ...
    nobodyPassport: (canton) => `Personne à ${canton} n'a de passeport chilien`,
    nobodyBorn: (canton) => `Personne à ${canton} n'est né au Chili`,
    leadNationals: (canton) => `Toutes les personnes de ${canton} avec un passeport chilien, selon chaque attribut du registre. Divisez par sexe pour descendre d'un niveau — le SEM ne va pas plus loin, il ne croise ces attributs qu'avec le sexe. Le BFS va plus loin : l'explorateur ci-dessous répond autorisation, sexe et âge à la fois.`,
    leadBorn: (canton, latam, rest) => `Toutes les personnes de ${canton} nées au Chili — un groupe plus grand et largement différent. Seules ${latam} conservent un passeport latino-américain ; les ${rest} autres ont la nationalité suisse, d'un pays de l'UE ou autre, et n'apparaissent dans aucun chiffre sur les ressortissants chiliens.`,
    popPassport: "Passeport chilien",
    popBorn: "Nés au Chili",
    everyone: "Tous",
    splitBySex: "Par sexe",
    women: "Femmes",
    men: "Hommes",
    ...
    bornMaritalNote: (year, total, head) => `L'état civil des personnes nées au Chili provient d'un autre cube et d'une année antérieure (31 déc ${year}) ; il compte donc ${total} et non ${head}. Les dates ne sont pas réconciliées.`,
  ...
  trend: {
    ...
    lead: (canton, small) => `Ressortissants chiliens à ${canton}, comptés par deux registres qui ne concordent pas et ne sont pas réconciliés ici.${small ? " À cette taille, une seule famille qui arrive déplace la courbe." : ""}`,
  ...
  movement: {
    ...
    leadSwiss: (grand, span) => `${grand} ressortissants chiliens sont devenus citoyens suisses ${span}. Chacun quitte le compte des passeports chiliens et rejoint celui des personnes nées au Chili — une grande partie de l'écart entre les deux.`,
  ...
  xf: {
    ...
    presetBornSwiss: "Nés au Chili avec passeport suisse",
    ...
    passportOfBorn: "Groupe de passeport · des personnes nées au Chili",
    any: "Tous",
  ...
  baselines: {
    eyebrow: "Comparaison",
    h: "Où vivent réellement les Chiliens",
    ...
    cardIn: (canton) => `Chiliens à ${canton}`,
    ...
    shareNational: "Part de tous les Chiliens en Suisse",
    ...
    axis: "ressortissants chiliens permanents",
    refTitle: "Moyenne nationale = 100",
    foot: "Le par-habitant utilise comme dénominateur le compte SEM de tous les résidents étrangers par canton (les totaux incluant les Suisses sont un concept BFS avec une autre date de référence). L'indice exprime le rapport Chiliens/étrangers de chaque canton relativement au rapport national.",
  },
  availability: {
  ...
  footer: {
    note: "Une exploration honnête des statistiques officielles sur les ressortissants chiliens et les résidents nés au Chili en Suisse, au niveau national et canton par canton. Construite uniquement à partir de données ouvertes collectées ; rien n'est estimé.",
    sources: "Sources",
